...
#    true_hits = jax.ops.segment_sum(jnp.ones_like(valid_indices), valid_indices, num_segments=max_index+1)[:max_index]
    ...
    # Combine the components
    return 3 * jnp.mean(weighted_time_differences) + jnp.mean(min_distances) + (jnp.sum(reco_hits)*num_photons-jnp.sum(true_hits))**2*jnp.mean((reco_hits*num_photons - true_hits) ** 2)

    #return jnp.mean((reco_hits*num_photons - true_hits) ** 2) #(jnp.sum(reco_hits)*num_photons/Nphot-jnp.sum(true_hits))**2

    #return (jnp.sum(reco_hits)*num_photons-jnp.sum(true_hits))**2*jnp.mean((reco_hits*num_photons - true_hits) ** 2)


    #return jnp.mean((reco_hits*num_photons/Nphot - true_hits) ** 2)
